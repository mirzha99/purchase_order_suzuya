...
class Po extends CI_Controller{
    ...
    public function po_barang($no_po = null){
        $check_no_po = $this->db->get_where('po_harga',['no_po'=>$no_po])->num_rows();
        $check_expired = $this->db->get_where('po',['no_po'=>$no_po])->row_object();
   
        if($no_po === null || $check_no_po === 0 || $check_expired->date_expired < time()){
            redirect('suplier/po');
        }
        ...
        $data['receiving_suplier'] = $this->receiving->receiving_no_po($no_po);
